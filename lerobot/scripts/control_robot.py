...
from lerobot.common.robot_devices.utils import busy_wait, safe_disconnect, safe_recover
...
def _resolve_dataset_root(raw_root: str | Path | None, repo_id: str) -> Path | None:
    """根据 repo_id 规范化数据输出目录."""

    if raw_root is None:
        return None

    base_path = Path(raw_root).expanduser()
    repo_parts = tuple(part for part in repo_id.split("/") if part)

    if repo_parts and len(base_path.parts) >= len(repo_parts):
        tail = base_path.parts[-len(repo_parts) :]
        if tuple(tail) == repo_parts:
            return base_path

    if repo_parts:
        base_path = base_path.joinpath(*repo_parts)
    return base_path
...
def record(
    ...
    dataset_root = _resolve_dataset_root(cfg.root, cfg.repo_id)
    if dataset_root is not None:
        dataset_root.parent.mkdir(parents=True, exist_ok=True)
        cfg.root = dataset_root
        logging.info(f"数据集输出目录：{cfg.root}")
    ...
    while True:
        ...
        episode_saved = events.get("episode_saved", False)
        episode_has_data = events.get("episode_has_data", False)

        need_reset = (
            not events["stop_recording"]
            and ((recorded_episodes < cfg.num_episodes - 1) or events["rerecord_episode"])
            and (episode_saved or episode_has_data or events["rerecord_episode"])
        )
        if need_reset:
...
def replay(
    ...
    # TODO(rcadene): Add option to record logs
    dataset_root = _resolve_dataset_root(cfg.root, cfg.repo_id)
    dataset = LeRobotDataset(cfg.repo_id, root=dataset_root, episodes=[cfg.episode])
