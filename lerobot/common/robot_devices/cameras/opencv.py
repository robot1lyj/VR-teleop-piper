...
class OpenCVCamera:
    ...
    def __init__(self, config: OpenCVCameraConfig):
        ...
        # 用于“图像 + 单调时间戳”的原子快照（避免读到撕裂状态）
        self._latest_frame_lock = threading.Lock()
        self._latest_frame: tuple[np.ndarray, int] | None = None
        self.logs = {}
    ...
    def read(self, temporary_color_mode: str | None = None) -> np.ndarray:
        ...
        # 录制严格对齐使用单调时钟（跨进程一致），而不是 UTC 时间
        t_ns = time.monotonic_ns()
        with self._latest_frame_lock:
            self._latest_frame = (color_image, int(t_ns))
    ...
    def async_read_with_timestamp(self) -> tuple[np.ndarray, int]:
        """异步读取最新帧并返回 (image, t_ns)。

        注意：t_ns 使用 ``time.monotonic_ns()``，且在采集线程里与 image 一起写入快照，
        便于录制端用同一时间基准对齐控制/测量 ring buffer。
        """
        ...
        while True:
            ...
            if num_tries > self.fps * 2:
                raise TimeoutError("Timed out waiting for async_read_with_timestamp() to start.")
    ...
    def async_read(self):
        ...
        image, _t_ns = self.async_read_with_timestamp()
        return image
